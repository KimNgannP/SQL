...
CREATE TRIGGER TRG_KIEMTRA_LOPTRUONG
ON LopHoc
AFTER INSERT, UPDATE
AS
BEGIN
    IF EXISTS (
        SELECT 1
        FROM LopHoc L
        INNER JOIN HocVien HV ON L.LopTruong = HV.MAHV
        WHERE HV.MALOP <> L.MALOP
    )
    BEGIN
        PRINT N'Lớp trưởng phải là học viên của lớp đó.';
        ROLLBACK TRAN;
    END
    ELSE
    BEGIN
        PRINT N'Kiểm tra lớp trưởng thành công.';
    END
END;

--10. Trưởng khoa phải là giáo viên thuộc khoa và có học vị “TS” hoặc “PTS”.
CREATE TRIGGER TRG_KIEMTRA_TRUONGKHOA
ON Khoa
AFTER INSERT, UPDATE
AS
BEGIN
    IF EXISTS (
        SELECT 1
        FROM Khoa K
        INNER JOIN GiangVien GV ON K.TruongKhoa = GV.MAGV
    ...
    ELSE
    BEGIN
        PRINT N'Kiểm tra trưởng khoa thành công.';
    END
END;
