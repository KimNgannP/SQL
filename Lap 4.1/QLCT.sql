...
FROM SkillLevels A
JOIN SkillLevels B ON A.MaChuyenGia < B.MaChuyenGia  -- tránh chọn cùng một chuyên gia và tránh trùng lặp
JOIN KyNang KN1 ON A.MaKyNang = KN1.MaKyNang
JOIN KyNang KN2 ON B.MaKyNang = KN2.MaKyNang
WHERE A.MaKyNang = B.MaKyNang  -- cả hai cùng kỹ năng
  AND A.SkillRank = 1          -- Chuyên gia A giỏi kỹ năng này nhất
  AND B.ReverseSkillRank = 1   -- Chuyên gia B kém kỹ năng này nhất
  AND A.CapDo > B.CapDo        -- A có cấp độ cao hơn B
